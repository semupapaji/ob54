...
async def Bmw(reg):
    ...
    async with httpx.AsyncClient() as cl:
        res = await cl.post(url, data=enc, headers={
            ...
            'X-GA': "v1 1",
            'ReleaseVersion': RELEASE_VERSION,  # Updated to OB54
            'X-FF-Client-Version': CLIENT_VERSION  # New header
        })
...
def OMG():
    ...
    if uid in UID_MEMORY:
        try:
            reg = UID_MEMORY[uid].get('region') if isinstance(UID_MEMORY[uid], dict) else UID_MEMORY[uid]
    ...
    for reg in REGNS:
        try:
            data = asyncio.run(LoL(uid, "7", reg, "/GetPlayerPersonalShow"))
            UID_MEMORY[uid] = {'region': reg, 'timestamp': time.time()}
            data = HeHe(data)
            return json.dumps(data, indent=2, ensure_ascii=False), 200, {'Content-Type': 'application/json; charset=utf-8'}
        except:
            continue
